  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`${backend}/api/settings`, {
        ...
        body: JSON.stringify({
          repositoryName: localConfig.repositoryName,
    ...
    <Card className="p-6 bg-card border-border shadow-card">
      ...
      <div className="space-y-6">
        <div className="space-y-3">
          <Label htmlFor="git-api-key" className="text-sm font-medium">GitHub API Key</Label>
          <Input
            id="git-api-key"
            type="password"
            placeholder="...."
        ...
        <div className="space-y-3">
          ...
          <Input
            id="repository-name"
            type="text"
            placeholder="ualers/Media-Cuts-Studio"
            value={localConfig.repositoryName}
            onChange={(e) => setLocalConfig({ ...localConfig, repositoryName: e.target.value })}
            className="font-mono text-sm bg-input border-border"
          />
        </div> 
