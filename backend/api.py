...
def health_check():
    ...
    try:
        ...
        GITHUB_TOKEN, REPOSITORY_NAME = get_tokens(numeric_user_id, log_action, logs_collection, SystemSettings, db)
...
def get_settings():
    ...
    try:
        ...
        response_data = {
            ...
            'repository_name': settings.repository_name or '',
...
def test_connection(service):
    ...
    try:
        ...
        if service == 'github':
            ...
            if not settings or not OPENAI_API_KEY:
...
def list_tasks():
    ...
    try:
        ...
        tasks_paginated = query.order_by(BackendTask.created_at.desc()).paginate(
            page=page, per_page=limit, error_out=False
        )
        ...
        return jsonify({'error': 'Falha ao buscar tarefas'}), 500
