...
from Agents.gpt_engineer.core.default.paths import metadata_path
from Agents.gpt_engineer.core.files_dict import FilesDict
from Agents.gpt_engineer.core.git import filter_by_gitignore, is_git_repo
...
class FileSelector:
    ...
    def ask_for_files(self, skip_file_selection=False) -> tuple[FilesDict, bool]:
        ...
        # --- Skip toda a parte interativa e de TOML quando solicitado ---
        if os.getenv("GPTE_TEST_MODE") or skip_file_selection:
            # Carrega todos os arquivos do projeto diretamente, sem seleção
            all_files = self.get_current_files(self.project_path)
            content_dict = {}
            for rel_path in all_files:
                full_path = Path(self.project_path) / rel_path
                try:
                   with open(full_path, "r", encoding="utf-8") as f:
                        content_dict[rel_path] = f.read()
                except (FileNotFoundError, UnicodeDecodeError):
                    # pula arquivos faltantes ou binários
                    continue
            # desabilita linting para não cair no pdb depois
