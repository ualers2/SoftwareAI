...
from Agents.gpt_engineer.core.base_execution_env import BaseExecutionEnv
from Agents.gpt_engineer.core.base_memory import BaseMemory
from Agents.gpt_engineer.core.chat_to_files import apply_diffs, chat_to_files_dict, parse_diffs
from Agents.gpt_engineer.core.default.constants import MAX_EDIT_REFINEMENT_STEPS
from Agents.gpt_engineer.core.default.paths import (
    CODE_GEN_LOG_FILE,
    DEBUG_LOG_FILE,
    DIFF_LOG_FILE,
    ENTRYPOINT_FILE,
...
def improve_fn(
    ...
    return _improve_loop(ai, files_dict, memory, messages, repo_path, diff_timeout=diff_timeout)
...
def _improve_loop(
    ...
    files_dict, errors = salvage_correct_hunks(
        messages, files_dict, memory, repo_path, diff_timeout=diff_timeout
    )
    ...
    while errors and retries < MAX_EDIT_REFINEMENT_STEPS:
        ...
        messages = ai.next(messages, step_name=curr_fn(), structured_output=True)
        files_dict, errors = salvage_correct_hunks(
            messages, files_dict, memory, repo_path, diff_timeout
        )
        retries += 1

    return files_dict

def salvage_correct_hunks(messages, files_dict, memory, repo_path, diff_timeout=3):
    import os, subprocess, tempfile
    from pathlib import Path

    error_messages = []
    diff_text = messages[-1].content.strip()
    filename = messages[-1].filename
    base_name = os.path.basename(filename)

    repo = Path(repo_path).resolve()
    if not (repo / base_name).exists():
        error_messages.append(f"Arquivo {base_name} não encontrado dentro de {repo}")
        return files_dict, error_messages

    # Garante cabeçalhos válidos
    if not diff_text.startswith("--- "):
        diff_text = f"--- {base_name}\n+++ {base_name}\n" + diff_text
    ...
    print(f"[diff] Aplicando via git apply em {repo}")

    try:
        # Primeiro tenta modo seguro 3-way
        result = subprocess.run(
            ["git", "apply", "--3way", "--ignore-space-change", "--whitespace=nowarn", tmp_path],
            cwd=repo,
            text=True,
            capture_output=True,
        )
        if result.returncode != 0:
            print(f"[diff] ⚠ Falha no modo 3way, tentando modo direto...")
            result = subprocess.run(
                ["git", "apply", "--ignore-space-change", "--whitespace=nowarn", "--reject", tmp_path],
                cwd=repo,
                text=True,
                capture_output=True,
            )
        ...
        if result.returncode == 0:
            print(f"[diff] ✅ Patch aplicado com sucesso em {base_name}")
        else:
            print(f"[diff] ❌ Falha ao aplicar patch:\n{result.stderr}")
...
#     messages: List, files_dict: FilesDict, memory: BaseMemory, repo_path='', diff_timeout=3
...
#         # Se o arquivo pré-existente não existe, criar vazio
#         if not diff.is_new_file() and diff.filename_pre not in files_dict:
#             files_dict[diff.filename_pre] = ""
