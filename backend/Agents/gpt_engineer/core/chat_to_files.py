...
from Agents.gpt_engineer.core.diff import ADD, REMOVE, RETAIN, Diff, Hunk
...
def chat_to_files_dict(chat: str) -> FilesDict:
    ...
    # REGEX COMBINADO:
    # 1. Estrutura Antiga: (\S+)\n\s*```[^\n]*\n(.+?)``` (Grupos 1 e 2)
    # 2. Estrutura ####: ####\s*`([^`]+)`\n\s*```[^\n]*\n(.+?)``` (Grupos 3 e 4)
    # 3. Estrutura ##: ##\s*\d*\.?\s*([^ \n]+)\n\s*```[^\n]*\n(.+?)``` (Grupos 5 e 6)
    # 4. Estrutura File: File:\s*([^ \n]+)\n\s*```[^\n]*\n(.+?)``` (Grupos 7 e 8)
    
    # Usamos grupos de não-captura (?:...) para agrupar as quatro estruturas.
    regex = (
        r"(?:(\S+)\n\s*```[^\n]*\n(.+?)```)|" # Padrão 1 (Grupos 1 e 2)
        r"(?:####\s*`([^`]+)`\n\s*```[^\n]*\n(.+?)```)|" # Padrão 2 (Grupos 3 e 4)
        r"(?:##\s*\d*\.?\s*([^ \n]+)\n\s*```[^\n]*\n(.+?)```)|" # Padrão 3 (Grupos 5 e 6)
        r"(?:File:\s*([^ \n]+)\n\s*```[^\n]*\n(.+?)```)" # Padrão 4 (Grupos 7 e 8)
    )
    
    # Agora temos 8 grupos de captura no total.
    matches = re.finditer(regex, chat, re.DOTALL)

    files_dict = FilesDict()
    for match in matches:
        
        # Lógica para encontrar o caminho (path) e o conteúdo (content) corretos
        # path será o primeiro grupo não-nulo entre G1, G3, G5, G7
        # content será o primeiro grupo não-nulo entre G2, G4, G6, G8

        # Encontra o Caminho (Path) - (Grupos 1, 3, 5, 7)
        path = (
            match.group(1) or 
            match.group(3) or 
            match.group(5) or 
        ...
        # Encontra o Conteúdo (Content) - (Grupos 2, 4, 6, 8)
        content = (
            match.group(2) or 
            match.group(4) or 
            match.group(6) or
        ...
        if not path or not content:
            continue # Pula se algo deu errado na extração

        # Clean and standardize the file path
        # O código de limpeza é robusto o suficiente para funcionar com o novo padrão:
        # Ele remove os caracteres inválidos e trata crases e colchetes.
        path = re.sub(r'[\:<>"|?*]', "", path)
...
def apply_diffs(diffs, files_dict, repo_path="."):
    """
    Aplica todos os diffs encontrados usando o DiffProcessor.
    Cada diff é salvo temporariamente e aplicado via `git apply --3way`.
    """
    applied = []
    failed = []

    for filename, diff_obj in diffs.items():
        try:
            # Salva o conteúdo do diff num arquivo temporário
            with tempfile.NamedTemporaryFile("w+", suffix=".diff", delete=False) as tmp:
                tmp.write(diff_obj.diff_to_string())
                tmp_path = Path(tmp.name)

            dp = DiffProcessor(tmp_path)
            dp.parse()
            dp.validate()
            dp.apply(repo_path)
            ...
            logger.error(f"❌ Falha ao aplicar diff em {filename}: {e}")

    if failed:
        logger.warning(f"⚠️ Alguns diffs falharam:\n" + "\n".join(failed))
    else:
        logger.info("✅ Todos os diffs foram aplicados com sucesso.")

    return files_dict  # mantido para compatibilidade
